...
import com.interviewcoach.tools.ResumeTool;
...
public class SpringAiServiceImpl implements SpringAiService {
     ...
     */
    @Override
    public BilingualResponse chatBilingual(String userId, String message, String sessionId) {
        String rawResponse = doChat(userId, message, chatModelConfig.getSystemPrompt(), sessionId);
        BilingualResponse response = BilingualResponseParser.parse(rawResponse);
        if (response == null || !response.isValid()) {
            String safeText = (rawResponse == null || rawResponse.isBlank()) ? "Let's continue the interview." : rawResponse;
            String english = safeText.substring(0, Math.min(500, safeText.length()));
     ...
     */
    @Override
    public BilingualResponse chatDynamicBilingual(String userId, String message, String stage, String focus, String sessionId) {
        String systemPrompt = buildDynamicSystemPrompt(userId, stage, focus);
        log.info("[AI-Dynamic] userId={}, stage={}, focus={}, sessionId={}", userId, stage, focus, sessionId);
        String rawResponse = doChat(userId, message, systemPrompt, sessionId);
        BilingualResponse response = BilingualResponseParser.parse(rawResponse);
        if (response == null || !response.isValid()) {
            String safeText = (rawResponse == null || rawResponse.isBlank()) ? "Let's continue the interview." : rawResponse;
            String english = safeText.substring(0, Math.min(500, safeText.length()));
    ...
    private String doChat(String userId, String userMessage, String systemMessage) {
        ...
        try {
            ...
            Prompt prompt;
            // 仅在使用默认 ChatModel 时开启 function calling（用户自定义模型可能不兼容）
            boolean useFunctionCallbacks = (model == chatModel)
                    && functionCallbacks != null && !functionCallbacks.isEmpty();
            if (useFunctionCallbacks) {
    ...
    /**
     * 带会话历史上下文的对话（面试场景使用）。
     * 从数据库加载最近 N 条历史消息拼入 prompt，让 LLM 感知上下文。
     */
    private String doChat(String userId, String userMessage, String systemMessage, String sessionId) {
        log.info("[AI] userId={}, sessionId={}, query={}", userId, sessionId,
                userMessage.substring(0, Math.min(userMessage.length(), 50)));
        ...
        try {
            ...
            // 加载会话历史
            int historyCount = loadConversationHistory(messages, sessionId);
            ...
            String content = model.call(prompt).getResult().getOutput().getContent();
            log.info("[AI] 响应长度: {} chars, 历史消息: {} 条", content != null ? content.length() : 0, historyCount);
            return content != null ? content : "";
        } catch (Exception e) {
            String rawMsg = e.getMessage() == null ? "unknown error" : e.getMessage();
            String lowerMsg = rawMsg.toLowerCase();
    ...
    /**
     * 从数据库加载会话最近的消息并拼入 messages 列表。
     * @return 加载的历史消息条数
     */
    private int loadConversationHistory(List<Message> messages, String sessionId) {
        if (sessionId == null || conversationService == null) return 0;
        try {
            List<InterviewMessage> history = conversationService.getMessagesBySessionId(sessionId);
            if (history == null || history.isEmpty()) return 0;

            // 取最近 10 轮对话作为上下文（太多会超 token 限制）
            int maxHistory = 10;
            ...
            log.info("[AI] 加载会话历史: sessionId={}, 总消息={}, 已加载={}", sessionId, history.size(), loaded);
            return loaded;
        } catch (Exception e) {
            log.warn("[AI] 加载会话历史失败: {}", e.getMessage());
            return 0;
        }
    }
    ...
    private ChatModel buildUserChatModel(String userId) {
        ...
        if (config == null || isBlank(config.getLlmType()) || Constants.PROVIDER_MOCK.equalsIgnoreCase(config.getLlmType())) {
            return null;
        }
        ...
        OpenAiChatOptions options = OpenAiChatOptions.builder()
                ...
                .build();
        return new OpenAiChatModel(api, options, functionCallbackContext, new RetryTemplate());
    }
